        ...
        //Si el comando introducido es "jobs"
            ...
            if(contadorJob != 0) {
                for(int i=0; i<maxJobs; i++) {  
                    if(jobs[i].active == 1) printf("[%d]+  Running\t\t%s", i + 1, jobs[i].command);
                    else if (jobs[i].active == 0) printf("[%d]-  Done\t\t%s", i + 1, jobs[i].command);
                }
            }
        }
        ...
        //Si es cualquier otro comando
            ...
            for(contador = 0; contador < line->ncommands; contador++) {
                ...
                //Padre
                    ...
                    pids[contador] = aux;
                    char aux_line[1024];
                    strcpy(aux_line, start_line);

                    //Si el proceso está en background se añade a la lista de procesos el nuevo proceso y actualizamos la lista de procesos
                    if(line->background == 1) addJob(aux, aux_line);
                }
...
void addJob(pid_t pid, char *line) {
    if(contadorJob < maxJobs) {
        ...
        strcpy(jobs[nextPosJob].command, line);
        jobs[nextPosJob].active = 1;
        contadorJob++;
    }
    else {
